<div <% if $Name %>id="$Name"<% end_if %><% if $isExpanded %> data-idsk-table-filter-expand="true"<% end_if %> class="idsk-table-filter__category<% if $extraClass %> $extraClass<% end_if %>">
	...
	<button class="govuk-body govuk-link idsk-filter-menu__toggle"
		...
		data-section-id="$Form.Name-$Name"
